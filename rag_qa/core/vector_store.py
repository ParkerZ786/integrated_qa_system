...
class VectorStore:
    ...
    def __init__(self,
        ...
        # 初始化 BGE-Reranker 模型，使用相对路径以避免 Windows 下非 ASCII 路径的问题
        reranker_rel_path = os.path.relpath(os.path.join(rag_qa_path, 'models', 'bge-reranker-large'))
        self.reranker = CrossEncoder(reranker_rel_path, device=self.device, tokenizer_args={'use_fast': False})
        # 初始化 BGE-M3 嵌入函数，同样使用相对路径
        m3_rel_path = os.path.relpath(os.path.join(rag_qa_path, 'models', 'bge-m3'))
        self.embedding_function = BGEM3EmbeddingFunction(model_name_or_path=m3_rel_path, use_fp16=(self.device == 'cuda'), device=self.device)
        # 获取稠密向量的维度# 1024
    ...
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            row = row.tocsr() if hasattr(row, 'tocsr') else row
            # 获取稀疏向量的非零值的索引
            indics = row.indices
            ...
            data.append({
                ...
                "dense_vector": embeddings["dense"][i].astype('float32'),
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        dense_query_vector = query_embeddings["dense"][0].astype('float32')
...
if __name__ == "__main__":
    ...
    # directory_path = r'C:\Users\21444\Desktop\work\黑马课程\第八阶段\eduRAG项目资料\资料\完整代码\integrated_qa_system\rag_qa\data\ai_data'
